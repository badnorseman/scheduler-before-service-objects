require "rails_helper"

describe BookingsController, type: :controller do
  before do
    @coach = create(:coach)
    @user = create(:user)
    @availability = create(:availability,
                           user: @coach,
                           start_at: Date.today,
                           end_at: Date.today + 1.week,
                           duration: 60,
                           beginning_of_business_day: Time.zone.parse("7:00AM"),
                           end_of_business_day: Time.zone.parse("11:00PM"),
                           auto_approving: true)
    @booking = create(:booking,
                      user: @user,
                      coach: @coach,
                      availability: @availability,
                      start_at: Time.zone.parse("12:00PM") + 1.day,
                      end_at: Time.zone.parse("1:00PM") + 1.day)
    create(:booking,
           user: @user,
           coach: @coach,
           availability: @availability,
           start_at: Time.zone.parse("11:00AM") + 1.day,
           end_at: Time.zone.parse("12:00PM") + 1.day)

    sign_in @user
  end

  describe "GET #index" do
    it "should query 2 Bookings" do
      get(
        :index
      )

      expect(json.count).to eq 2
    end
  end

  describe "GET #show" do
    it "should read 1 Booking" do
      get(
        :show,
        id: @booking.id
      )

      expect(json["start_at"]).to eq(@booking.start_at.as_json)
    end
  end

  describe "POST #create" do
    context "with valid attributes" do
      before do
        @booking_attributes =
          attributes_for(:booking,
                         start_at: Time.zone.parse("9:00AM") + 1.day,
                         availability_id: @availability.id,
                         coach_id: @coach.id)
      end

      it "should create Booking" do
        expect do
          post(
            :create,
            booking: @booking_attributes
          )
        end.to change(Booking, :count).by(1)
      end

      it "should be automatically approved" do
        post(
            :create,
            booking: @booking_attributes
          )

        expect(json["confirmed_at"]).to be_between(Time.zone.now - 1.minute, Time.zone.now)
      end
    end

    context "with invalid attributes" do
      it "should not create Booking" do
        @booking_attributes =
          attributes_for(:booking,
                         start_at: Time.zone.parse("6:00AM") - 1.day,
                         availability_id: @availability.id,
                         coach_id: @coach.id)
        expect do
          post(
            :create,
            booking: @booking_attributes
          )
        end.to change(Booking, :count).by(0)
      end
    end
  end

  describe "PATCH #update" do
    context "with valid attributes" do
      it "should update Booking" do
        random_start_at = @booking.start_at + rand(5).hours

        patch(
          :update,
          id: @booking.id,
          booking: { start_at: random_start_at }
        )

        expect(Booking.find(@booking.id).start_at).to eq(random_start_at)
      end
    end

    context "with invalid attributes" do
      it "should not update Booking" do
        patch(
          :update,
          id: @booking.id,
          booking: { start_at: @booking.start_at - 30.minutes }
        )

        expect(Booking.find(@booking.id).start_at).to eq(@booking.start_at)
      end
    end
  end

  describe "DELETE #destroy" do
    it "should delete Booking" do
      expect do
        delete(
          :destroy,
          id: @booking.id
        )
      end.to change(Booking.where("cancelled_at IS NULL"), :count).by(-1)
    end
  end
end
